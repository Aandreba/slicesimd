...
pub fn benchmark_reduce_add(c: &mut Criterion) {
    let input = thread_rng()
        .sample_iter(Uniform::new(-100.0, 100.0f32))
        .take(1_000_000)
        .collect::<Vec<_>>();

    c.bench_with_input(
        BenchmarkId::new("simdslice w/ copy", 0),
        &input,
        |b, input| {
            b.iter(|| {
    ...
    c.bench_with_input(
        BenchmarkId::new("simdslice w/o copy", 1),
        &input,
        |b, input| {
            let mut simdslice = input.clone();
    ...
    c.bench_with_input(
        BenchmarkId::new("rayon simdslice (w/o copy)", 3),
        &input,
        |b, input| {
            let mut simdslice = input.clone();
            ...
            b.iter(|| {
                let mut result = simdslice
                    .par_chunks_mut(len)
                    .map(reduce_add_in_place)
                    .collect::<Vec<_>>();
